...
exports.saveProfile = async (req, res) => {
  try {
    console.log("✅ registerController.saveProfile HIT");
    ...
    if (dob) {
      ...
      const thisYearBirthday = new Date(today.getFullYear(), dobDate.getMonth(), dobDate.getDate());
      const age = today.getFullYear() - dobDate.getFullYear() - (today < thisYearBirthday ? 1 : 0);
    ...
    // ── ATOMIC UPSERT SAVE ────────────────────────────────────
    // findOneAndUpdate with upsert:true → finds user OR creates if missing
    // runValidators:true  → enforces schema rules on update
    // new:true            → returns the updated document
    const updatedUser = await User.findOneAndUpdate(
      { phone: fullPhone },                      // 🔍 match by phone
      {
        $set: {
          ...
          gender:    gender  || "",
          photo:     photo   || "",
          isVerified: true,                      // keep verified flag intact
        },
      },
      {
        new: true,           // return updated doc
        upsert: true,        // create user if not found (safety net)
        runValidators: true, // honour schema validations
      }
    );
    ...
    // Mongoose validation error → send field-level detail
